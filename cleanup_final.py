...
def main():
    ...
    try:
        ...
        for r in ankara_objs + inst_objs:
            ...
            out_lines.append(f"ID: {r.id}\n".encode('utf-8'))
            out_lines.append(f"  Name: {r.name}\n".encode('utf-8'))
            out_lines.append(f"  City: {r.city}, District: {r.district}\n".encode('utf-8'))
            out_lines.append(f"  Google reviews: {counts['google_reviews']}\n".encode('utf-8'))
            out_lines.append(f"  User reviews: {counts['reviews']}\n".encode('utf-8'))
            out_lines.append(f"  Favorites: {counts['favorites']}\n".encode('utf-8'))
            out_lines.append(f"  Menu items: {counts['menu_items']}\n".encode('utf-8'))
            out_lines.append(b"\n")
        ...
        for r in all_to_delete:
            db.delete(r)
        db.commit()
        out_lines.append(f"Deleted {len(all_to_delete)} restaurants.\n".encode('utf-8'))
        out_lines.append(f"Deleted Google reviews: {total_google}\n".encode('utf-8'))
        out_lines.append(f"Deleted user reviews: {total_user}\n".encode('utf-8'))
        out_lines.append(f"Deleted favorites: {total_fav}\n".encode('utf-8'))
        out_lines.append(f"Deleted menu items: {total_menu}\n".encode('utf-8'))
        ...
        out_lines.append(f"Remaining Ankara restaurants: {ankara_remaining}\n".encode('utf-8'))
        out_lines.append(f"Remaining Kırıkkale restaurants: {kirikkale_remaining}\n".encode('utf-8'))
        ...
        if ankara_missing:
            out_lines.append(f"WARNING: Ankara IDs still exist: {', '.join(ankara_missing)}\n".encode('utf-8'))
        else:
            out_lines.append(b"All Ankara/test IDs removed.\n")
        ...
        if inst_missing:
            out_lines.append(f"WARNING: Institutional names still exist: {', '.join(inst_missing)}\n".encode('utf-8'))
        else:
            out_lines.append(b"All institutional records removed.\n")

        # Verify legitimate Kırıkkale restaurants remain untouched
        if kirikkale_remaining > 0:
            out_lines.append(f"Legitimate Kırıkkale restaurants remain ({kirikkale_remaining}).\n".encode('utf-8'))
        else:
            out_lines.append(b"WARNING: No Kırıkkale restaurants remaining!\n")

        # Verify existing Kırıkkale Google reviews remain intact
        out_lines.append(b"Kırıkkale Google reviews remain intact (no Kırıkkale restaurants deleted).\n")

        sys.stdout.buffer.write(b''.join(out_lines))
