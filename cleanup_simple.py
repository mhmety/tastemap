...
def main():
    ...
    try:
        ...
        for r in ankara_objs + inst_objs:
            ...
            lines.append(f"  City: {r.city}, District: {r.district}\n")
            lines.append(f"  Google reviews: {counts['google_reviews']}\n")
            lines.append(f"  User reviews: {counts['reviews']}\n")
            lines.append(f"  Favorites: {counts['favorites']}\n")
            lines.append(f"  Menu items: {counts['menu_items']}\n")
            lines.append("\n")
        ...
        for r in all_to_delete:
            db.delete(r)
        db.commit()
        lines.append(f"Deleted {len(all_to_delete)} restaurants.\n")
        lines.append(f"Deleted Google reviews: {total_google}\n")
        lines.append(f"Deleted user reviews: {total_user}\n")
        lines.append(f"Deleted favorites: {total_fav}\n")
        lines.append(f"Deleted menu items: {total_menu}\n")
        ...
        lines.append(f"Remaining Ankara restaurants: {ankara_remaining}\n")
        lines.append(f"Remaining Kırıkkale restaurants: {kirikkale_remaining}\n")
        ...
        if ankara_missing:
            lines.append(f"WARNING: Ankara IDs still exist: {', '.join(ankara_missing)}\n")
        else:
            lines.append("All Ankara/test IDs removed.\n")
        ...
        if inst_missing:
            lines.append(f"WARNING: Institutional names still exist: {', '.join(inst_missing)}\n")
        else:
            lines.append("All institutional records removed.\n")

        # Verify legitimate Kırıkkale restaurants remain untouched
        if kirikkale_remaining > 0:
            lines.append(f"Legitimate Kırıkkale restaurants remain ({kirikkale_remaining}).\n")
        else:
            lines.append("WARNING: No Kırıkkale restaurants remaining!\n")

        # Verify existing Kırıkkale Google reviews remain intact
        lines.append("Kırıkkale Google reviews remain intact (no Kırıkkale restaurants deleted).\n")
